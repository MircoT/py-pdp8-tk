...
class pdp8(object):
    ...
    def _ADD(self):
        ...
        if self.tempo == 0:
            ...
            self.E = (
                "1"
                if (self.AC[0] == "1" and self.MBR[0] == "1" and int(temp, 2) < 98304)
                or (self.AC[0] == "0" and self.MBR[0] == "0" and int(temp, 2) > 32767)
                else "0"
            )
            self.AC = temp[1:]
    ...
    def carica(self, codice):
        ...
        if temp[0] == "ORG":
            if int(str(temp[1]), 16) > -1 and int(str(temp[1]), 16) < 4096:
                ...
                avviso(
                    "Errore di caricamento, ORG di inizio file non corretto!",
                    style=ICON_ERROR,
                )
                return None
        else:
        ...
        try:
            ...
            avviso(
                "Errore di caricamento, Fine codice assembly (END) non trovato!",
                style=ICON_ERROR,
            )
            return None
        ...
        try:
            ...
            avviso(
                "Errore di caricamento, correggere la seguente parola : " + str(y),
                style=ICON_ERROR,
            )
            raise e
            return None
